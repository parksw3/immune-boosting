...
\jd{Revisit:} Throughout the paper, we start by assuming that a population mixed homogeneously and that there is no loss of immunity.
...
\jd{You may have confused yourself by using 1/2?. We should have the same formula for all cases (second s.~below can be deleted, or made into a parenthetical). $1-\phi$ does not appear (when $\phi\to0$ all of the vax classes $\to0$); it should be $\phi$ below.}
...
\jd{I'm a bit worried about this; how can we get more info.}
...
\jd{Why interesting? It's directly connected to the assumption above, among other things.}
...
The $S_v$ compartment is not included in the polarized vaccination model because it represents a set of individuals who have not retained any immunity from vaccination. \jd{see parallel note, note also that this gives us trouble with limits if we are thinking about limits}
...
\jd{WHAT? \VE\ on x-axis, right? We assume a combined efficacy of $\VE = 0.6$ throughout.}
All other parameters are the same as in \fref{simulation}.
\jd{Yellow in r-h panels is misleading. Can we make black N/A or something? Are all of the others (non-black) colors?}
...
\jd{This is getting deep, I guess we should start by assuming that observation is perfect, no?}
...
\jd{I might need to be talked through some of this; I'm leaning towards leaving hazard-based stuff mostly out. We can say that it gives the right answer in theory, but is hard to do and we have the problem of estimating susceptibles... Actually, I now think that we really don't need any of this. Effectiveness should be based on who was vaccinated (within some time frame, but we can ignore that), not what we think their immune status is.}
...
\jd{No, we definitely need to discuss. I at least need to understand 4b before we decide what to do about it!}
