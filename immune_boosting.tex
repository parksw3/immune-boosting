...
Two epidemiological models of vaccination have been proposed.
...
\jd{I would prefer to use “immune-boosting” as an adjective, and save “immune boosting” for when (if?) we use it as a noun. Happy to do the edit if you don't object.}
...
\jd{We list them just above and in the figure, I prefer not to list them here.}
As explained earlier, the leaky vaccination model predicts more cases among vaccinated individuals than the other two models, which predict identical incidence trajectories. \jd{Saying incidence here is nice, and supports the idea that this is what we should plot.}
...
\jd{Completely irrelevant note. If we all had the practice of talking about proportional changes as log-ratios, then we would say that the relative likelihood of people being infected (or polarized to unprotected) was $\exp(\VE)$ -- and then the $\VE$ equation becomes $\VE = \VE_L + \VE_P$! It would be so cool.}
...
(A--C) \jd{Any objection to showing incidence instead of Prevalence? I feel it's a small point where modelers tend to diverge unnecessarily from practicioners.} Prevalence of infection among unvaccinated ($I_u$, blue solid) and vaccinated ($I_v$, orange dashed) individuals.
...
In particular, the immune boosting model predicts identical epidemic dynamics to the polarized vaccination model because individuals in both cases are completely immune after surviving a single challenge. 
...
Yet formalization of how the two extreme models commonly used (leaky and polarized) are related has been lacking.
By developing this formalism, we both illustrate the spectrum of trajectories expected for a range of configurations, alongside an investigation of what this means and how this can be interpreted in the context of vaccine effectiveness.
