...
and $\VE_L$ represents the vaccine efficacy, which captures the amount of reduction in the probability of infection.
...
For this model, the parameter $\VE_P$ also provides some sort of measure for vaccine efficacy but has a different meaning from the parameter $\VE_L$ in the leaky vaccine model---we return to this point later.
...
Here, we do not distinguish the source of antibodies (whether derived from natural infections or vaccinations) and assume that individuals in $R_u$, $S_v$, and $R_v$ compartments are seropositive, except in the case of polarized vaccination:
in the case of polarized vaccination, we do not expect individuals in the $S_v$ compartment to be seropositive because they have not retained any immunity from the vaccination.
