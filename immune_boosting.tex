...
\jd{Invite Mike Li? Would be nice to include him more, and he played an influential role in getting me to start moving this forward.}
...
\jd{I am trying to think about how to discuss some really basic issues, specifically homogeneous mixing and no loss of immunity. Maybe with a ¶ below the model and \emph{before} we mention that the vaccine is leaky?}

\jd{It's pretty cool that I can just type the paragraph mark. I still haven't found a way to make tex recognize real greek letters, though.}

%% \jd{$α$ does not work!}
%% \jd{Plain α also does not work!}
...
\jd{This seems too strong, let's discuss.}
...
\jd{What about multiply infected people? Probably best to deal with this above (directly address that we are modeling a “single epidemic” which specifically means neglecting loss of immunity and multiple infections.}
...
\jd{Should we talk about time interval?}
To calculate the reduction in cumulative incidence in our model, we add two additional compartments, which keep track of cumulative incidence among unvaccinated $C_u$ and vaccinated $C_v$ individuals:
\begin{align}
