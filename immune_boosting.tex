...
To address this latter assumption, we introduce an immune boosting mechanism, through which partially susceptible individuals can gain protection without developing a transmissible infection.
...
Recent efforts to develop vaccines against COVID-19 have raised important questions about how vaccine efficacy and effectiveness should be estimated \citep{kennedy2021estimating,lipsitch2021interpreting,kahn2022identifying,follmann2023vaccine,kahn2024examining} and interpreted \citep{lipsitch2020understanding,imai2021interpreting}.
In parallel, there has been considerable discussion around whether COVID-19 vaccines should be modeled as ``leaky'' or ``all-or-nothing'' (polarized) \citep{han2021time,kahn2022identifying,liu2022optimizing,miura2023inferring,otto2024endemic}.
On one hand, the leaky vaccination model assumes that vaccinated individuals experience a reduced probability of infection per exposure (e.g., multiplied by a factor $1-\VEL < 1$, where \VEL\ represents (leaky) vaccine efficacy \citep{smith1984assessment}).
On the other hand, the ``all-or-nothing'' (polarized) vaccination model assumes that the proportion \VEP\ of vaccinated individuals are completely protected and the remaining proportion $1-\VEP$ of vaccinated individuals are completely susceptible, where \VEP\ represents (polarized) vaccine efficacy \citep{gog2002dynamics,gomes2014missing}.
A few studies suggested that qualitative conclusions about vaccine rollouts can be insensitive to these assumptions \citep{bubar2021model,keeling2023impacts}.
However, understanding the characteristics of partial protection (i.e., leaky vs polarized) is essential to accurately predicting future outbreaks and estimating vaccine efficacy and effectiveness because leaky vaccine always predicts a larger outbreak \citep{smith1984assessment}.
...
So far, many models for COVID-19 vaccines have relied on the leaky assumption (see \cite{liu2022optimizing} for a review).
Although this assumption is supported by empirical evidence for exposure-dependent protection \citep{lind2023evidence}, current approaches to modeling laeky vaccination relies on an implicit assumption: 
individuals in this model do not lose any susceptibility when (implicitly) exposed to a challenge that does not result in infection. 
This implicit assumption in particular overlooks a potentially important mechanism:
vaccinated individuals who successfully fight off exposures can experience immune boosting, thus becoming less susceptible to future infections without becoming infectious or developing symptoms from the exposure \citep{lavine2011natural,yang2020waning}.
