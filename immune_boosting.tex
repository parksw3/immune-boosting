...
The leaky vaccination model assumes that vaccinated individuals experience a reduced force of infection (e.g., multiplied by a factor $1-\VEL < 1$, where \VEL\ represent vaccine efficacy, which we define as the proportion of people protected from their first challenge).
The ``all-or-nothing'' vaccination model assumes that the proportion \VEP\ of vaccinated individuals are completely protected and the remaining proportion $1-\VEP$ of vaccinated individuals are completely susceptible, where \VEP\ also represents vaccine efficacy.
This model is analogous to the “polarized” model of cross-immunity, in which infection from one strain gives complete or no protection against other strains \citep{gog2002dynamics}---we thus refer to this model as the polarized vaccination model \citep{gomes2014missing}.
...
\jd{I don't really love this wording, but otherwise we would need to reword substantially -- I don't think we can say no loss of immunity “corresponds” to a short time scale, because there are plenty of diseases where immunity is not lost.}
