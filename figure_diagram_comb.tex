...
\begin{tikzpicture}[node distance=2.5cm,auto,>=latex',every node/.append style={align=center},int/.style={draw, minimum size=1cm, circle},inverter/.style={rectangle,draw,inner sep=2pt,minimum size=6mm},]
    ...
    \path[->, line width=1.2mm, shorten >= 16pt, shorten <= 16pt] (tmp1) edge node [above] {$\bm{q=0, \VE_P=0}$} (tmp2);
    \path[->, line width=1.2mm, shorten >= 4pt, shorten <= 4pt] (Rv2) edge node [sloped, above] {$\bm{q=1, \VE_P=0}$} (S3);
    \path[->, line width=1.2mm, shorten >= 4pt, shorten <= 4pt] (Rs2) edge node [sloped, below] {$\bm{q=0, \VE_L=0}$} (V4);
    ...
    \path[-{Straight Barb[left]}, line width=1.2mm, shorten >= 30pt, shorten <= 30pt] ([xshift=1mm]Iv4.south) edge node [sloped, above=0.2cm] {\textbf{Equivalent transmission}\\\textbf{dynamics but different}\\\textbf{serological dynamics}\\\textbf{when }$\bm{\VE_P=\VE_L}$} ([xshift=1mm]Is3.north);
